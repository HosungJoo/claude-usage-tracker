...
기본은 **주 모니터** 한가운데입니다. `workArea` 기준이라 상단 패널이나 독을
침범하지 않습니다.

> **Wayland에서는 앱이 마우스 위치를 알 수 없습니다.** 그래서 '커서가 있는
> 화면'이 기본이 아닙니다. Electron은 XWayland의 포인터를 읽는데, XWayland는
> 자기 창 위에 있을 때만 포인터를 봅니다. 마우스가 네이티브 Wayland 창
> (GNOME Terminal, Tilix 등)으로 넘어가면 좌표가 **마지막 위치에 멈춥니다.**
>
> 실측했습니다. 마우스를 다른 모니터에서 크게 움직이는 6초 동안
> `XQueryPointer` 는 한 픽셀도 변하지 않았고, 그 좌표는 반대편 모니터에 있는
> VS Code 창 안쪽이었습니다. 캐릭터가 계속 엉뚱한 화면에 뜬 이유입니다.

설정에서 모니터를 직접 고를 수 있습니다 — 방향과 해상도로 표시되므로
어느 것인지 알아볼 수 있습니다.
