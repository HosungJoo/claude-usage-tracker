...
Claude Code의 마스코트 **Claw'd** 입니다. 원본 아트워크에서 픽셀 단위로
그대로 옮겼습니다 — 16×10 격자, 코랄 단색, 외곽선도 음영도 없습니다.
그 절제가 캐릭터의 인상을 만들기 때문에 아무것도 더하지 않았습니다.
...
이 표는 테스트에 그대로 들어 있습니다. 렌더링이 여기서 벗어나면 실패합니다.

표정은 원본이 가진 부품만으로 만듭니다 — **눈 슬릿의 높이와 위치, 팔의 각도**.
입을 새로 그리지 않는 이유도 같습니다. 원본에 없는 것을 더하는 순간
다른 캐릭터가 됩니다.

| 표정 | 눈 | 팔 | 쓰이는 곳 |
|---|---|---|---|
| `idle` | 2칸 | 제자리 | 기본 |
| `blink` | 1칸(아래) | 제자리 | 4.2초마다 깜박 |
| `happy` | 1칸(위) | 살짝 듦 | 여유 있을 때 |
| `talk` | 2칸 | 번갈아 듦 | 보고 중 |
| `wave` | 2칸 | 한쪽 번쩍 | 세션 시작 인사 |
| `worry` | 2칸(처짐) | 축 처짐 | 주의 · 70% |
| `panic` | 3칸 | 두 팔 번쩍 | 위험 · 90%+ |
...
src/shared/
  ...
  character/palette.ts     팔레트 (코랄 + 눈, 둘뿐)
